...
export const ProjectCatalog: React.FC<ProjectCatalogProps> = ({
  ...
  const bgImageUrl = `${baseUrl}${baseUrl.endsWith('/') ? '' : '/'}images/spatial-observatory-bg.jpg`;
    ...
    >
      ...
      <header className="relative z-20 w-full px-6 sm:px-12 py-5 sm:py-6 flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3.5">
          {/* Wireframe 3D Cube Icon */}
          <div className="w-10 h-10 rounded-2xl bg-cyan-500/10 border border-cyan-400/40 flex items-center justify-center text-cyan-300 shadow-[0_0_20px_rgba(6,182,212,0.3)] backdrop-blur-md">
            <Box className="w-5 h-5 stroke-[2.2]" />
          </div>

          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="text-base sm:text-lg font-black tracking-[0.2em] text-white uppercase drop-shadow-[0_2px_12px_rgba(255,255,255,0.25)]">
                XR MODEL VIEWER
              </span>
              <span className="hidden md:inline-flex items-center gap-1 text-[9px] uppercase font-bold tracking-wider bg-cyan-950/80 text-cyan-300 border border-cyan-500/30 px-2 py-0.5 rounded-full backdrop-blur-md">
                <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
                {isMetaQuest ? 'Quest 3S' : 'Web 3D'}
              </span>
            </div>
            <span className="text-[10px] sm:text-[11px] font-semibold tracking-[0.28em] text-cyan-400/90 uppercase">
              VISUALIZA · EXPLORA · APRENDE
            </span>
          </div>
        </div>
      ...
      {/* 7. Interactive Quick View Modal (360° Preview + Snapshot Camera) */}
      <QuickViewModal
      ...
      {/* 8. Floating Scroll Buttons for Meta Quest Laser Pointer Navigation */}
