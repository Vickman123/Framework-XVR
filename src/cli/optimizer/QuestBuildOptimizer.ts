...
export class QuestBuildOptimizer {
  ...
  private static readonly MAX_RECOMMENDED_GLB_BYTES = 25 * 1024 * 1024; // 25 MB
  private static readonly MAX_RECOMMENDED_TEXTURE_BYTES = 4 * 1024 * 1024; // 4 MB
  private static readonly MAX_RECOMMENDED_TOTAL_BYTES = 100 * 1024 * 1024; // 100 MB
   ...
   */
  public static analyzeDirectory(dirPath: string, rootDir: string = dirPath): QuestAuditReport {
    const assets: AssetAuditItem[] = [];
    ...
    const scan = (current: string) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          let type: AssetAuditItem['type'] = 'other';
          ...
          if (['.glb', '.gltf'].includes(ext)) {
            ...
            if (size > QuestBuildOptimizer.MAX_RECOMMENDED_GLB_BYTES) {
              itemWarnings.push(
                `Modelo 3D pesado (${QuestBuildOptimizer.formatBytes(size)} > 25MB). Puede causar picos de memoria en Meta Quest Browser.`
              );
              largeFilesCount++;
            }
            ...
            if (size > QuestBuildOptimizer.MAX_RECOMMENDED_TEXTURE_BYTES) {
              itemWarnings.push(
                `Textura pesada (${QuestBuildOptimizer.formatBytes(size)} > 4MB). Se recomienda limitar a 2048x2048 o comprimir a KTX2/WebP.`
              );
              largeFilesCount++;
            }
    ...
    if (totalSizeBytes > QuestBuildOptimizer.MAX_RECOMMENDED_TOTAL_BYTES) {
      warnings.push(
        `El tamaño total del proyecto (${QuestBuildOptimizer.formatBytes(totalSizeBytes)}) excede los 100MB recomendados para carga rápida en Meta Quest Browser.`
      );
    }

    if (glbCount > 10) {
      recommendations.push(
        `Se detectaron ${glbCount} archivos GLB/GLTF. Para Quest, se recomienda combinar geometrías estáticas o utilizar carga diferida (lazy loading).`
      );
    }

    if (textureTotalBytes > 40 * 1024 * 1024) {
      recommendations.push(
        `El peso acumulado de texturas (${QuestBuildOptimizer.formatBytes(textureTotalBytes)}) es alto. Considera compresión KTX2 Basis Universal.`
      );
    }

    // Recommended render parameters for Quest
    recommendations.push(
      'Configura pixelRatioCap <= 1.25 en XRAppOptions para asegurar 90 FPS estables sin sobrecalentamiento del visor.'
    );
    recommendations.push(
      'Limita mapas de sombras a máximo 1024x1024 y usa 1 sola luz direccional con castShadow activo.'
    );

    let questBudgetScore: QuestAuditReport['questBudgetScore'] = 'Optimal';
   ...
   */
  public static generateQuestManifest(name: string = 'VXR WebXR Experience'): object {
    return {
      name,
   ...
   */
  public static generateQuestDeployGuide(projectName: string = 'VXR App'): string {
